 ...
 public:
  Drone(Vector<3> ctr, MatrixRot<3> ort)
      : CoordS(ctr, ort, nullptr),
        ...
        rotor({
            HexaP(Vector<3>({0.5, 1, 0.25}), MatrixRot<3>(), &this->dBody, 0.5, 0.2),
            HexaP(Vector<3>({-0.5, 1, 0.25}), MatrixRot<3>(), &this->dBody, 0.5, 0.2),
            HexaP(Vector<3>({0.5, -1, 0.25}), MatrixRot<3>(), &this->dBody, 0.5, 0.2),
            HexaP(Vector<3>({-0.5, -1, 0.25}), MatrixRot<3>(), &this->dBody, 0.5, 0.2),
        }),
        searchlight({HexaP(Vector<3>({0.25, 1.05, 0}),
                           MatrixRot<3>(90, Axis::OX), &this->dBody, 0.15, 0.1),
                     HexaP(Vector<3>({-0.25, 1.05, 0}),
                           MatrixRot<3>(90, Axis::OX), &this->dBody, 0.15, 0.1)}),
